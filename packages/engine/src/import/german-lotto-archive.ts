 ...
 */
export function looksLikeGermanLottoArchive(csv: string): boolean {
  const firstLine = csv
    .replace(/^\uFEFF/, "")
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function nonEmptyCells(cells: readonly string[]): string[] {
  return cells.filter((cell) => cell.length > 0);
}

function parseGermanDate(value: string, sourceRow: number): string {
  const match = value.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/);
  if (!match) {
    throw new CsvImportError(
      `Invalid German date "${value}". Expected DD.MM.YYYY.`,
      sourceRow,
    );
  }
...
function isNoiseRow(cells: readonly string[]): boolean {
  if (cells.length === 0) {
    return true;
  }
 ...
 */
export function parseGermanLottoArchive(csv: string): readonly ParsedDraw[] {
  const lines = csv
    .replace(/^\uFEFF/, "")
    .split(/\r?\n/);
  ...
  if (draws.length === 0) {
    throw new CsvImportError(
      "German Lotto archive contains no valid draw rows.",
    );
  }
...
export function germanLottoArchiveToStandardCsv(csv: string): string {
