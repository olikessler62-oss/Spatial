...
export class CsvParser {
  public parse(input: string, options: CsvParserOptions = {}): readonly ParsedDraw[] {
      ...
      .map((row, index) => {
        ...
        return {
          ...
          ...(externalIdIndex >= 0 && row[externalIdIndex]?.trim()
            ...
            : {}),
        } satisfies ParsedDraw;
      });
  }

  private parseRows(input: string, delimiter: string): string[][] {
    if (delimiter.length !== 1) throw new CsvImportError("Delimiter must be exactly one character.");
    ...
    if (quoted) throw new CsvImportError("CSV contains an unterminated quoted field.");
    if (value.length > 0 || row.length > 0) { row.push(value); rows.push(row); }
    return rows;
  }

  private requiredColumn(header: readonly string[], column: string): number {
    const index = header.indexOf(column);
    if (index < 0) throw new CsvImportError(`Required column "${column}" is missing.`);
    return index;
  }

  private requiredValue(row: readonly string[], index: number, column: string, sourceRow: number): string {
    const value = row[index]?.trim();
    if (!value) throw new CsvImportError(`Required value "${column}" is missing.`, sourceRow);
    return value;
  }

  private normalizeDate(value: string, sourceRow: number): string {
  ...
  private parseNumbers(value: string, separator: string, sourceRow: number, column: string): readonly number[] {
    const parts = value.split(separator).map((part) => part.trim()).filter(Boolean);
    if (parts.length === 0) throw new CsvImportError(`Column "${column}" contains no numbers.`, sourceRow);
    return parts.map((part) => {
      if (!/^\d+$/.test(part)) throw new CsvImportError(`Column "${column}" contains invalid number "${part}".`, sourceRow);
      return Number(part);
    });
  }
}
