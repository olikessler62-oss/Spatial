...
function countHits(sequence: readonly boolean[], window: number): number {
 ...
 */
export function computePlacementHistoryFeatures(
  hitSequence: readonly boolean[],
): PlacementHistoryFeatures {
  ...
  let drawsSinceLastHit: number;
  let averageHistoricalGap: number;
  let maximumHistoricalGap: number;
  let currentGapRatio: number;
  ...
  const recentHitRate =
    recentWindow === 0 ? 0 : countHits(hitSequence, recentWindow) / recentWindow;
...
export function featuresToMetricValues(
  features: PlacementHistoryFeatures,
...
export const EXPERIMENT1_RANKING_CRITERIA = [
  { metricId: "gap-ratio", weight: 0.5, direction: "descending" as const },
  { metricId: "recent-inactivity", weight: 0.3, direction: "descending" as const },
  { metricId: "historical-frequency", weight: 0.2, direction: "descending" as const },
];
