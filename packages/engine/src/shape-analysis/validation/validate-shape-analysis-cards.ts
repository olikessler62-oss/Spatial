 ...
 */
export function validateShapeAnalysisCard(
  card: ShapeAnalysisCard,
): void {
  if (card.rowCount <= 0 || card.columnCount <= 0) {
    throw new ShapeAnalysisError(
      "INVALID_GRID_DIMENSIONS",
      `Card "${card.id}" has invalid grid dimensions ${card.rowCount}x${card.columnCount}.`,
      {
        cardId: card.id,
  ...
  if (card.cells.length !== expectedCellCount) {
    throw new ShapeAnalysisError(
      "INCOMPLETE_GRID",
      `Card "${card.id}" expects ${expectedCellCount} cells but has ${card.cells.length}.`,
      {
        cardId: card.id,
  ...
  for (const cell of card.cells) {
    if (
      ...
      throw new ShapeAnalysisError(
        "INCOMPLETE_GRID",
        `Card "${card.id}" has a cell outside the grid at (${cell.row}, ${cell.column}).`,
        {
          cardId: card.id,
          row: cell.row,
          column: cell.column,
    ...
    if (seen.has(key)) {
      throw new ShapeAnalysisError(
        ...
        `Card "${card.id}" has a duplicate cell coordinate (${cell.row}, ${cell.column}).`,
        {
          cardId: card.id,
          row: cell.row,
          column: cell.column,
  ...
  if (seen.size !== expectedCellCount) {
    throw new ShapeAnalysisError(
      "INCOMPLETE_GRID",
      `Card "${card.id}" is missing cell coordinates.`,
      {
        cardId: card.id,
        expected: String(expectedCellCount),
 ...
 */
export function validateShapeAnalysisCards(
  ...
  const inconsistent = cards.find(
    (card) =>
      card.rowCount !== selectedCard.rowCount ||
  ...
  if (inconsistent !== undefined) {
    throw new ShapeAnalysisError(
      ...
      `Card "${inconsistent.id}" has layout ${inconsistent.rowCount}x${inconsistent.columnCount}, expected ${selectedCard.rowCount}x${selectedCard.columnCount}.`,
      {
