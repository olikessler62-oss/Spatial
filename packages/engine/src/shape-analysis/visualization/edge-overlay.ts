...
export function buildEdgeContributions(
  cardId: string,
  occurrences: readonly VisibleShapeOccurrence[],
): readonly ShapeEdgeContribution[] {
  const contributions: ShapeEdgeContribution[] = [];
  ...
  for (const occurrence of occurrences) {
    ...
    const edgeKeys = getRectangleEdgeKeysCached(
      occurrence.geometryKey,
      occurrence.geometry,
    );
    const cssColor = resolveShapeCssColor(
      occurrence.color,
...
function compareDominance(
  a: ShapeEdgeContribution,
  b: ShapeEdgeContribution,
): number {
...
export function renderGridEdges(
  cardId: string,
  occurrences: readonly VisibleShapeOccurrence[],
  _configuration: ShapeVisualizationConfiguration,
): readonly RenderedGridEdge[] {
  const contributions = buildEdgeContributions(cardId, occurrences);
  const byEdge = new Map<string, ShapeEdgeContribution[]>();
  ...
  const rendered: RenderedGridEdge[] = [];

  for (const [edgeKey, list] of byEdge) {
    const shapeIds = [...new Set(list.map((item) => item.shapeId))].sort(
