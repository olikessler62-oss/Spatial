 ...
 */
export const SHAPE_COLOR_PALETTE: readonly ShapeVisualColor[] = [
 ...
 */
export function getShapeVisualColor(seed: string): ShapeVisualColor {
 ...
 */
export function assignDistinctShapeColors(
  seeds: readonly string[],
): ReadonlyMap<string, ShapeVisualColor> {
  const unique = [...new Set(seeds)].sort((a, b) => a.localeCompare(b));
  const bySeed = new Map<string, ShapeVisualColor>();

  for (let index = 0; index < unique.length; index += 1) {
    const seed = unique[index]!;
    bySeed.set(
      seed,
      SHAPE_COLOR_PALETTE[index % SHAPE_COLOR_PALETTE.length]!,
    );
  }
...
export function getShapeColorKey(seed: string): string {
  const color = getShapeVisualColor(seed);
  return `hsl:${color.hue}:${color.saturation}:${color.baseLightness}`;
}

export function resolveShapeCssColor(
  color: ShapeVisualColor,
