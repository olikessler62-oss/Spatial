...
export function findProbabilityAtLeast(
  runLength: number,
  survivalDistribution: readonly RunLengthSurvivalEntry[],
): number | null {
  if (survivalDistribution.length === 0) {
    return null;
  }

  const exact = survivalDistribution.find(
    (entry) => entry.runLength === runLength,
  );
  ...
  const maxHistorical = Math.max(
    ...survivalDistribution.map((entry) => entry.runLength),
  );
  ...
  let best: RunLengthSurvivalEntry | undefined;
...
export function compareAgainstModes(
  currentRunLength: number,
  modeRunLengths: readonly number[],
  ...
  let relation: ModeComparison["relation"];
  ...
  const distanceToNearestMode = Math.min(
    ...sorted.map((mode) => Math.abs(mode - currentRunLength)),
  );
...
export function compareAgainstMedian(
  currentRunLength: number,
...
export function compareAgainstAverage(
  currentRunLength: number,
...
export function compareAgainstMaximum(
  currentRunLength: number,
  maximumRunLength: number,
): MaximumComparison {
  let relation: MaximumComparison["relation"];
