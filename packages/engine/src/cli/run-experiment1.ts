...
function syntheticDraws(count: number): ParsedDraw[] {
...
function main(): void {
  ...
  const csvPath =
    csvIndex >= 0 ? process.argv[csvIndex + 1] : undefined;
  ...
  console.log(
    `Running Experiment1 holdout multi-seed (${seedCount}) on ${draws.length} draws…`,
  );
