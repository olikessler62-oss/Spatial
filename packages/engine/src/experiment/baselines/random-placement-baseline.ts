...
export function runRandomPlacementBaseline(options: {
  ...
  for (const seed of seeds) {
    ...
    for (
      ...
      const anyHit = chosen.some(
        (placement) =>
          evaluator.evaluate(placement, draw).hitCount >= minHits,
      );
...
function pickRandomPlacements(
  placements: readonly IndexedPlacement[],
  topK: number,
  random: () => number,
): IndexedPlacement[] {
...
export function empiricalPValue(
  ...
  baseline: BaselineDistributionSummary,
): number {
  if (baseline.hitRates.length === 0) {
    return 1;
  }

  const asExtreme = baseline.hitRates.filter(
    (rate) => rate >= observedHitRate,
  ).length;
