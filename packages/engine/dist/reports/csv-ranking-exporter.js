export class CsvRankingExporter {
    ...
    exportRanking(ranking, options = {}) {
        const entries = options.limit === undefined
            ? ranking.entries
            : ranking.entries.slice(0, options.limit);
        const metricIds = ranking.appliedCriteria.map((criterion) => criterion.metricId);
        const header = [
            "rank",
            "resultId",
            "score",
        ...
        const rows = entries.map((entry) => {
            const criterionByMetricId = new Map(entry.criteria.map((criterion) => [
                criterion.metricId,
                criterion,
            ]));
            return [
...
//# sourceMappingURL=csv-ranking-exporter.js.map