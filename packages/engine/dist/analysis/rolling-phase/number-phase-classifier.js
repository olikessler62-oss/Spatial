function relativeDelta(windowValue, globalValue) {
    if (windowValue === null
        || globalValue === null
        || globalValue === 0
...
export function classifyNumberPhase(input) {
    ...
    const globalMedianEstimate = stats.median !== null && medianDiff !== null
        ? stats.median - medianDiff
        : null;
    if (globalMedianEstimate !== null
        && stats.median !== null
    ...
    const globalTrimmedEstimate = stats.trimmedMean10Percent !== null && trimmedDiff !== null
        ? stats.trimmedMean10Percent - trimmedDiff
        : null;
    if (globalTrimmedEstimate !== null
        && stats.trimmedMean10Percent !== null
        && globalTrimmedEstimate !== 0) {
        const rel = (stats.trimmedMean10Percent - globalTrimmedEstimate)
    ...
    void medianDelta;
    const strongTrend = input.hitRateTrend.direction !== "stable"
        || input.medianMissStreakTrend.direction !== "stable";
    const previousDistinct = new Set(input.previousPhaseTypes.filter((phase) => phase !== "insufficient-data" && phase !== "transition"));
    let phaseType;
    ...
    const classificationStrength = phaseType === "transition"
...
//# sourceMappingURL=number-phase-classifier.js.map