...
function requiredEnvironment(name) {
    ...
    if (!value) {
        throw new Error(`Missing environment variable ${name}. Copy .env.example to .env and configure it.`);
    }
    return value;
}
...
function parseOptions() {
    ...
    if (!file) {
        throw new Error("Missing --file. Example: npm run import:csv -- --file ../../datasets/samples/lotto-6aus49.csv");
    }
    return {
        file,
        datasetId: getArgument("dataset-id")
            ?? process.env.SPATIAL_DATASET_ID
            ?? SEEDED_DATASET_ID,
        ruleSetId: getArgument("rule-set-id")
...
function printRejectedRows(rejectedDraws) {
...
async function main() {
    ...
    if (report.rejectedRows > 0) {
        console.error(`Import rejected: ${report.rejectedRows} of ${report.receivedRows} rows are invalid.`);
