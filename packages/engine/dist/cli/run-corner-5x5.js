...
function main() {
    ...
    console.log(JSON.stringify({
        ...
        byShape: [...byShape.entries()].map(([shape, items]) => ({
            ...
            avgCurrentMissStreak: items.reduce((sum, item) => sum + item.currentMissStreak, 0)
                / items.length,
            avgHistoricalMissRate: items.reduce((sum, item) => sum + item.historicalMissRate, 0)
                / items.length,
        })),
...
main();
//# sourceMappingURL=run-corner-5x5.js.map