...
import { MaximalEmptyRectangleFinder, fullGridSearchArea, } from "../detection/rectangle/maximal-empty-rectangle-finder.js";
import { createRectangleGeometryKey, rectangleCellCount, } from "../detection/rectangle/rectangle-geometry.js";
import { buildThroughSelectedSequence, countIgnoredNewerCards, } from "../analysis-window.js";
...
export class CurrentShapePersistenceEngine {
    dependencies;
    maximalFinder = new MaximalEmptyRectangleFinder();
    ...
    analyze(request) {
        ...
        const analysisCards = buildThroughSelectedSequence(request.cards, request.selectedCardId);
        ...
        const distanceFromSelectedByCardId = new Map(analysisCards.map((card, index) => [card.id, index]));
        ...
        if (request.enabledShapeTypes.includes("rectangle")) {
            const searchArea = fullGridSearchArea(newestCard.rowCount, newestCard.columnCount);
            for (let lookback = minBasis; lookback <= analysisCards.length; lookback += 1) {
                this.throwIfAborted(request);
                const basisCards = analysisCards.slice(analysisCards.length - lookback);
                const intersection = buildIntersectionHitMatrix(basisCards);
                const prefixSum = buildHitPrefixSum(intersection);
                const geometries = this.maximalFinder.find(prefixSum, searchArea, request.minimumShapeCellCount);
                for (const geometry of geometries) {
                    ...
                    for (const sub of enumerateSubRectangles(geometry, request.minimumShapeCellCount)) {
        ...
        const nodes = new Map();
        const sortedKeys = [...candidates.keys()].sort((a, b) => a.localeCompare(b));
        for (const geometryKey of sortedKeys) {
            this.throwIfAborted(request);
            const geometry = candidates.get(geometryKey);
            const streak = this.measureStreak(geometry, geometryKey, analysisCards, emptyCache);
            ...
            const reachesOldestBound = streak.oldestFreeCardId === oldestCard.id
            ...
            shapeEntries.push({
                ...
                ...(status === "terminated"
                    ? { terminationReason: "hit-inside-shape" }
                    : {}),
                parentIds: [],
                childIds: [],
            ...
            for (let i = 0; i < newestFirst.length; i += 1) {
                ...
                occurrences.add({
                    ...
                    distanceFromSelectedCard: distanceFromSelectedByCardId.get(cardId) ?? i,
                });
                if (i === 0) {
                    events.push({
        ...
        return {
            ...
            metadata: {
                ...
                ignoredNewerCardCount: countIgnoredNewerCards(request.cards, request.selectedCardId),
    ...
    measureStreak(geometry, geometryKey, cardsNewestLast, emptyCache) {
        const newest = cardsNewestLast[cardsNewestLast.length - 1];
        const occurrenceCardIds = [];
        let oldestFree = newest;
        for (let i = cardsNewestLast.length - 1; i >= 0; i -= 1) {
            const card = cardsNewestLast[i];
            const empty = this.isGeometryEmpty(geometry, geometryKey, card, emptyCache);
            if (!empty) {
                return {
                    occurrenceCardIds,
    ...
    isGeometryEmpty(geometry, geometryKey, card, cache) {
    ...
    throwIfAborted(request) {
        if (request.executionOptions?.signal?.aborted) {
            throw new ShapeAnalysisError("ANALYSIS_CANCELLED", "Shape persistence analysis was cancelled.");
        }
    }
}
function isRect(geometry) {
...
export function enumerateSubRectangles(outer, minimumCellCount) {
    const result = [];
...
//# sourceMappingURL=current-shape-persistence-engine.js.map