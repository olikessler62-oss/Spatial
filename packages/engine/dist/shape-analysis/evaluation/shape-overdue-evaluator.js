...
import { buildSurvivalDistribution, calculateHistoricalRunPercentile, } from "../statistics/run-statistics.js";
import { compareAgainstAverage, compareAgainstMaximum, compareAgainstMedian, compareAgainstModes, findProbabilityAtLeast, } from "./shape-run-comparison.js";
import { DEFAULT_SHAPE_OVERDUE_EVALUATION_CONFIGURATION, SHAPE_OVERDUE_DISCLAIMER_CODES, } from "./shape-overdue-types.js";
export class DefaultShapeOverdueEvaluator {
    evaluate(request) {
        ...
        const runsForStats = configuration.useCompletedRunsOnly
            ? request.historical.completedRuns
            : request.historical.runs;
        const percentile = runsForStats.length === 0
            ? null
            : calculateHistoricalRunPercentile(currentRunLength, runsForStats);
        const percentageShorterThanCurrent = percentile === null ? null : percentile.percentageShorterThan / 100;
        const percentageAtMostCurrent = percentile === null ? null : percentile.percentageAtMost / 100;
        const survival = configuration.useCompletedRunsOnly &&
        ...
        const probabilityAtLeastCurrentLength = runsForStats.length === 0
            ? null
            : findProbabilityAtLeast(currentRunLength, survival);
        const insufficient = request.historical.completedRunCount <
        ...
        if (configuration.compareAgainstMode &&
            request.historical.mode !== null) {
            modeComparison = compareAgainstModes(currentRunLength, request.historical.mode.runLengths, request.historical.mode.frequency);
        ...
        if (configuration.compareAgainstMedian &&
            request.historical.medianRunLength !== null) {
            medianComparison = compareAgainstMedian(currentRunLength, request.historical.medianRunLength);
        ...
        if (request.historical.averageRunLength !== null) {
            averageComparison = compareAgainstAverage(currentRunLength, request.historical.averageRunLength);
        }
        let maximumComparison = null;
        if (configuration.compareAgainstMaximum &&
            request.historical.maximumRunLength !== null) {
            maximumComparison = compareAgainstMaximum(currentRunLength, request.historical.maximumRunLength);
        ...
        if (dataQualityStatus === "insufficient" ||
        ...
        return {
            ...
            dataQuality: {
                ...
                historicalDataContainsCensoring: request.historical.censoredRunCount > 0,
    ...
    validateKeys(request) {
        ...
        if (current.shapeType !== historical.key.shapeType ||
            current.geometryKey !== historical.key.geometryKey ||
            layoutKey !== historical.key.layoutKey) {
            throw new ShapeAnalysisError("SHAPE_STATISTICS_KEY_MISMATCH", "Current persistence entry and historical statistics refer to different shapes.", {
    ...
    validateCurrent(current) {
        if (current.coveredCardCount < 1) {
            throw new ShapeAnalysisError("INVALID_CURRENT_RUN", "coveredCardCount must be at least 1.");
        }
        if (current.previousCardCount !== current.coveredCardCount - 1) {
            throw new ShapeAnalysisError("INVALID_CURRENT_RUN", "previousCardCount must equal coveredCardCount - 1.");
        }
    }
    validateHistorical(historical) {
        if (historical.completedRunCount !== historical.completedRuns.length) {
            throw new ShapeAnalysisError("INVALID_HISTORICAL_STATISTICS", "completedRunCount does not match completedRuns.length.");
        }
    }
    validateConfiguration(configuration) {
        const { elevatedPercentileThreshold: elevated, rarePercentileThreshold: rare, extremePercentileThreshold: extreme, minimumCompletedRunCount, } = configuration;
        if (elevated < 0 ||
            elevated > 1 ||
            rare < 0 ||
            rare > 1 ||
            extreme < 0 ||
            extreme > 1) {
            throw new ShapeAnalysisError("INVALID_EVALUATION_CONFIGURATION", "Percentile thresholds must be between 0 and 1.");
        }
        if (!(elevated < rare && rare < extreme)) {
            throw new ShapeAnalysisError("INVALID_EVALUATION_CONFIGURATION", "Thresholds must satisfy elevated < rare < extreme.");
        }
        if (minimumCompletedRunCount < 0) {
            throw new ShapeAnalysisError("INVALID_EVALUATION_CONFIGURATION", "minimumCompletedRunCount must not be negative.");
        }
    }
}
export function evaluateShapeOverdueBatch(request, evaluator = new DefaultShapeOverdueEvaluator()) {
    const evaluations = request.entries.map((entry) => evaluator.evaluate({
...
export function summarizeShapeOverdueBatch(evaluations) {
    ...
    const scored = evaluations.filter((evaluation) => evaluation.overdueScore !== null);
    const highestScore = scored.length === 0
        ? null
        : Math.max(...scored.map((evaluation) => evaluation.overdueScore));
    const highestScoringShapeKeys = highestScore === null
        ? []
        : scored
...
const CLASSIFICATION_SORT_RANK = {
    "historical-maximum-exceeded": 0,
...
export function compareShapeOverdueEvaluations(a, b) {
    const rankDiff = CLASSIFICATION_SORT_RANK[a.classification] -
...
function rectangleAreaFromKey(geometryKey) {
...
//# sourceMappingURL=shape-overdue-evaluator.js.map