...
import { assignDistinctShapeColors, } from "./shape-color.js";
import { DEFAULT_SHAPE_VISUALIZATION_CONFIGURATION, createOccurrenceVisualizationKey, } from "./visualization-types.js";
export function buildShapeVisualizationPlan(result, configuration = DEFAULT_SHAPE_VISUALIZATION_CONFIGURATION, analysisId = `analysis-${result.selectedCardId}-${result.shapes.length}`) {
    const colorByGeometryKey = assignDistinctShapeColors(result.shapes.map((shape) => shape.geometryKey));
    const colorByShapeId = new Map();
    const coveredByShapeId = new Map();
    const geometryByShapeId = new Map(result.shapes.map((shape) => [
        shape.shapeId,
        {
    ...
    for (const shape of result.shapes) {
        const color = colorByGeometryKey.get(shape.geometryKey)
    ...
    for (const occurrence of result.occurrences) {
        ...
        const occurrenceKey = createOccurrenceVisualizationKey(occurrence.shapeId, occurrence.cardId);
        occurrenceByKey.set(occurrenceKey, {
            ...
            color,
            brightnessLevel: computeShapeBrightnessLevel(occurrence.distanceFromSelectedCard, covered, configuration.brightness),
    ...
    const steps = [];
    const knownOccurrenceKeys = new Set();
    const pushStep = (actions) => {
        if (actions.length === 0) {
            return;
        }
        steps.push({
    ...
    const brightnessUpdatesForShape = (shapeId) => {
        const updates = [...occurrenceByKey.values()]
            .filter((occurrence) => occurrence.shapeId === shapeId)
            .filter((occurrence) => knownOccurrenceKeys.has(occurrence.occurrenceKey) ||
    ...
    for (const event of result.events) {
        switch (event.type) {
            case "initial-shape-detected": {
                const key = createOccurrenceVisualizationKey(event.shapeId, event.cardId);
