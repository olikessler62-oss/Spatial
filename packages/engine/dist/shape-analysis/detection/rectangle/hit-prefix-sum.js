export function buildHitPrefixSum(hitMatrix) {
    const rowCount = hitMatrix.length;
    const columnCount = hitMatrix[0]?.length ?? 0;
    const prefix = Array.from({ length: rowCount + 1 }, () => Array.from({ length: columnCount + 1 }, () => 0));
...
export function countHitsInRectangle(prefixSum, geometry) {
...
export function isEmptyRectangle(prefixSum, geometry) {
    return countHitsInRectangle(prefixSum, geometry) === 0;
}
//# sourceMappingURL=hit-prefix-sum.js.map