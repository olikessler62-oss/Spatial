 ...
 */
export function buildHitMatrix(card) {
    validateShapeAnalysisCard(card);
    const matrix = Array.from({ length: card.rowCount }, () => Array.from({ length: card.columnCount }, () => false));
 ...
 */
export function buildIntersectionHitMatrix(cards) {
    if (cards.length === 0) {
        throw new ShapeAnalysisError("EMPTY_CARD_SET", "Intersection hit matrix requires at least one card.");
    }
    ...
    for (const matrix of matrices) {
        if (matrix.length !== rowCount
            ...
            throw new ShapeAnalysisError("INCONSISTENT_GRID_LAYOUT", "Intersection requires identical grid dimensions on every card.");
        }
    }
    return Array.from({ length: rowCount }, (_, row) => Array.from({ length: columnCount }, (_, column) => matrices.some((matrix) => matrix[row]?.[column] === true)));
}
/**
 * Geometrically equivalent cards share the same hit matrix
 * regardless of lottery number placement.
 */
export function hitMatricesEqual(a, b) {
...
//# sourceMappingURL=hit-matrix.js.map